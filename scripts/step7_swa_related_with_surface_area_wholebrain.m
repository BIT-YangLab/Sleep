...
network_size_variability = cell(network_n, 1);

R2_list = zeros(network_n, 4);



p_list_org = zeros(network_n, 1);
for networki = 1:network_n
    ...
    p_list_org(networki, 1) = p;
    network_size_variability{networki, 1} = temp_size_variability;
...
for networki = 4:4%network_n
    ...
    temp_size_variability = network_size_variability{networki};
    draw_plot(temp_size_variability, {'SWA'}, label_name{networki}, stageNames{1+1}, ['/nd_disk3/guoyuan/Jinlong/sleep_ret/HFR_ret/pic/swa_related_' num2str(network_n) '_net' num2str(networki) '_' mask_name_list_w{mask_idx} '.png'], p_list_org(networki, 1));
    draw_plot_clean(temp_size_variability, {'SWA'}, label_name{networki}, stageNames{1+1}, ['/nd_disk3/guoyuan/Jinlong/sleep_ret/HFR_ret/pic/swa_related_' num2str(network_n) '_net' num2str(networki) '_' mask_name_list_w{mask_idx} '_clean.png'], p_list_org(networki, 1));
...
function draw_plot(data, behavior_name, network_name, stage_name, out_name, p_fdr)
...
function draw_plot_clean(data, behavior_name, network_name, stage_name, out_name, p_fdr)
