	...
	for subi = 1:size(network_label_all, 2)
		...
		for sessi = 1:length(network_label_all{stagei, subi})
			...
			for networki = 1:network_n
				...
				network_size_list(networki) = length(idx) / length(label_tmp);
			end
...
for classIdx = 1:4
    ...
    Y_interp = interp1(X, Y, X_macro, 'linear', 'extrap');  % ÏßÐÔ²åÖµ
...
save(['/nd_disk3/guoyuan/Jinlong/sleep_ret/HFR_ret/SVM_multiclass_all_50_rpt_classify_network' num2str(network_n) '_' mask_name_list_w{mask_idx} '.mat' ], ...
 	'sleep_stage_classify_acc', 'externalConfusionMatrix_all', ...
     'sleep_stage_classify_beta', 'sleep_stage_classify_bias', ...
 	'null_sleep_stage_classify_acc', ...
 	'ROC_x',  'ROC_y', 'AUC_list', 'ROC_x_multi', 'ROC_y_multi', 'AUC_multi');
