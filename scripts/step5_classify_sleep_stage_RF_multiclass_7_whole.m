...
% multiple classification for sleep stage with network size ratio by RF
% 
clc;clear;
...
for balance_i = 1:balance_rpt_time
    ...
    [externalAccuracies, externalConfusionMatrix, allTrueLabels, allPredictedScores] = RF_predict_stage(feature_data, rept_svm, kfold, nTrees, ...
    '');
...
for balance_i = 1:balance_rpt_time
    ...
    for rpti = 1:null_rept
        ...
        [externalAccuracies, externalConfusionMatrix, ~, ~] = RF_predict_stage(feature_temp, 1, kfold, nTrees, '');
...
save(['/nd_disk3/guoyuan/Jinlong/sleep_ret/HFR_ret/RF_multiclass_' mask_name_list_w{mask_idx} '_50_rpt_classify_network' num2str(network_n) '.mat' ], ...
...
function [externalAccuracies, externalConfusionMatrix, allTrueLabels, allPredictedScores] = RF_predict_stage(data, nRepeat, kfold_time, nTrees, ROC_file_stem)
    ...
    for repeat = 1:nRepeat
        ...
        for i = 1:cv.NumTestSets
            ...
            % training model
            rfModel = TreeBagger(nTrees, X(trainIdx, :), Y(trainIdx), 'Method', 'classification');
            
            % Predictive testing set
            [predictedLabels, scores] = predict(rfModel, X(testIdx, :));
