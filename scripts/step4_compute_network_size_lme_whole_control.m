...
% compute individual network size for control group
% 
clc;clear;
...
for mask_i = 1:length(mask_list_w)
    for stai = 1:4
        ...
        for subi = 1:length(network_label_all)
            ...
            if nnz(control_sub_list_select{stai} == subi) == 0
                continue;
            end
    
            temp_size_sta = zeros(1, network_n);
            
            network_tmp_label = network_label_all{stai, subi}{1};
    ...
    for neti = 1:network_n
        ...
        writetable(t1, ['/nd_disk3/guoyuan/Jinlong/sleep_ret/HFR_ret/text/Network_size_table_control_brainstructure_' num2str(mask_i) '_net' num2str(neti) '.csv']);
    end
