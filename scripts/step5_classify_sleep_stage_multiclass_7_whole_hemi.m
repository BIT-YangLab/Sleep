...
% features: network size ratio and divide the left and right hemisphere
...
for stagei = 1:4
	for subi = 1:size(network_label_all, 2)
		...
		for sessi = 1:length(network_label_all{stagei, subi})
            network_size_list = zeros(1, network_n*2+1);
            for hemi_i = 1:2
                label_tmp = network_label_all{stagei, subi}{sessi};
                label_tmp = label_tmp(mask_list{(mask_idx-1)*2+hemi_i});
                
                for networki = 1:network_n
                    idx = find(label_tmp == networki);
                    network_size_list(networki+(hemi_i-1)*network_n) = length(idx) / nnz(mask_list{(mask_idx-1)*2+hemi_i});
                end
            end
            network_size_list(networki*2 + 1) = stagei;
...
for balance_i = 1:balance_rpt_time
    ...
    [externalAccuracies, externalConfusionMatrix, beta_Matrix, bias_Matrix] = svm_predict_stage(feature_data, rept_svm, kfold, ...
    '');
...
save(['/nd_disk3/guoyuan/Jinlong/sleep_ret/HFR_ret/SVM_multiclass_' mask_name_list_w{mask_idx} '_hemisphere_50_rpt_classify_network' num2str(network_n) '.mat' ], ...
 	'sleep_stage_classify_acc', 'sleep_stage_classify_confMatrix', ...
     'sleep_stage_classify_beta', 'sleep_stage_classify_bias');


function [externalAccuracies, externalConfusionMatrix, beta_Matrix, bias_Matrix] = svm_predict_stage(data, nRepeat, kfold_time, ROC_file_stem)
