...
% cleanup for subcortex atlas constructed with single sessions
% 
...
for stagei = 1:4
    for si = 1:size(subcortex_label, 2)
        ...
        for sesi = 1:length(subcortex_label{stagei, si})
            label1 = subcortex_label{stagei, si}{sesi};
            ...
            disp(['change ratio:' num2str(nnz(new_label_assign ~= subcortex_label{stagei, si}{sesi}) / 96854)]);
            subcortex_label{stagei, si}{sesi} = new_label_assign;
        end
        toc;
    end
end
