...
% visualization for group average atlas - control group
...
load([ret_dir '/IndiPar_net_multi_7/Network_ind_subcortex5min.mat']);
...
for stagei = 1:4
    ...
    if nnz(control_sub_list_select{stagei} == si) == 0
        continue;
    end
        label1 = [label1 subcortex_label{stagei, si}{1}];
    end
    label_average = mode(label1, 2);
    group_label_list{stagei} = label_average;

    plot_subcortex(label_average, [ret_dir '/buckner_test.nii.gz'], [ret_dir '/pic/Subcortex_control_lisa_stage_' stageNames{stagei}]);
    create_MNI_dlabel(label_average, [ret_dir '/MNI152_control_subcortex_lisa_visualization_stage_' stageNames{stagei} '.dlabel.nii']);
    create_cortex_label_dlabel(label_average, [ret_dir '/sleep_network_control_' stageNames{stagei}  '.dlabel.nii'], Yeo_7Networks_Color);
end


%% hemisphere group network size
group_size_list = zeros(4, 8);
group_size_list_w = zeros(4, 4);
for mask_idx = 1:4
