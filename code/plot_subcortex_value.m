function plot_subcortex_value(label, nii_file, out_fig_base_dir)
% plot subcortex
% Author: Xinyu Wu and Guoyuan Yang, BIT.
% Modify by Jinlong Li
% 
    ...
    cmap = hot;
     ...
     plot_fig_subcortex(nii_file, 'export_fig_path', age_grad_hiptha_png_path, 'cmap', cmap, 'cscale', [min_scale, max_scale], 'nifti_roi_label', [10, 49]);
...
%     camorbit(95, -8, 'data', [0, 0, 1]);
    ...
    plot_fig_subcortex(nii_file, 'export_fig_path', age_grad_str_png_path, 'cmap', cmap, 'cscale', [min_scale, max_scale], 'nifti_roi_label', [11, 12, 13, 26, 50, 51, 52, 58]);
    camorbit(39, 0, 'data', [0, 0, 1]);
    export_fig(age_grad_str_png_path, '-m6', '-q100');
    close;

    age_grad_cbm_png_path = [out_fig_base_dir '_cbm.png'];
    map = suit_map2surf(nii_file, 'space', 'SPM', 'stats',@minORmax);
    set(gcf, 'Position', [0, 0, 1000, 1000]);
    set(gca, 'xticklabel', []); set(gca, 'yticklabel', []); set(gca, 'zticklabel', []);
    set(gca, 'color', 'none'); set(gcf, 'color', 'none');
    axis off;
    suit_plotflatmap(map, 'cmap', cmap,'threshold',min_scale, 'cscale', [min_scale, max_scale]);
